...
const LoginPage = () => {
  ...
  // Focus on email input after the component mounts
  useEffect(() => {
    if (userRef.current) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/users/login', JSON.stringify({ email, password }), {
  ...
  return (
    ...
    <>
      ...
      {success ? (
        <div className='relative h-screen w-[100vw] justify-center items-center'>
        ...
        <div className='flex flex-col h-auto w-full bg-baseextra4 items-center justify-center'>
          <div className="w-full bg-white rounded-lg shadow dark:border sm:max-w-md xl:p-0 m-4">
            <div className="p-6 space-y-4 sm:p-8">
              ...
              {errMsg && <div ref={errRef} className="text-red-500 text-sm mb-4">{errMsg}</div>}
              <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                <div>
                  ...
                  <input
                    ...
                    name="email"
                    id="email"
                    ref={userRef} // Attach userRef to the email input
